...
def get_parser():
    parser = ArgumentParser(description="LLM을 사용하여 코드 패치를 순차적으로 생성합니다. (개선된 버전)")
...
def score_candidate(row: Dict[str, Any], code: str, ast_pass: bool) -> int:
    ...
    try:
        ...
        if "pass" in code or "..." in code or "NotImplementedError" in code:
            score -= 50
        # BANNED_TOKENS 리스트를 여기서 직접 확인하여 페널티 부과
...
class QwenClient:
    ...
    def generate(self, prompts: List[str], gen_args: Dict[str, Any]) -> List[str]:
        ...
        inputs = self.tokenizer(all_chats, return_tensors="pt", padding=True, truncation=True).to(self.model.device)
...
def process_batch(client: QwenClient, batch: List[Tuple[str, Dict[str, Any]]], args: Namespace):
    ...
    for state in batch_states:
        ...
        for cand in all_candidates:
            # 수정된 score_candidate 함수를 통해 모든 후보가 유연하게 평가됨
